...
M = '\x1b[1;37m'
B = '\x1b[38;5;208m'
YIKIM = random.randint(100, 300)
O = f'\x1b[38;5;{YIKIM}m'
...
Geliştirici: @Tasdemir46
Kanalımız: @@cehennemCC
"""

def HYPERIOS_TOOL_PYTHON_REPORT():
    os.system("cls" if os.name == "nt" else "clear")
    print(Fore.GREEN + Style.BRIGHT + title + Style.RESET_ALL)
    Banner = f"""{X}{X}======================================================{X}
|{E}[+] Tool  : {B} Tasdemir46 Tool - Instagram Reporter
|{E}[+] Server: {B} Web Telegram | https://
{X}====================================================== \n
"""
...
HYPERIOS_TOOL_PYTHON_REPORT()

user = input(f' {B}({E}1{B}) {X} Hedef Kullanıcı Adını Girin IG{X} :  ' + O)
print(X + ' ═════════════════════════════════  ')
ses = input(f' {F}({E}2{F}) {X} Session ID’nizi Girin IG{F} :  ' + O)
csr = token_hex(8) * 2
console = Console()
bb = 0
gg = 0
...
while True:
    ...
    os.system('clear')
    table = Table(title="Tasdemir - Instagram Reporter")
    table.add_column("Tür", justify="center", style="cyan", no_wrap=True)
    table.add_column("Sayı", justify="center", style="magenta")
    table.add_row("Başarılı Rapor", str(gg), style="green")
    table.add_row("Başarısız Rapor", str(bb), style="red")
    table.add_row("Hedef Kullanıcı", user, style="white")
    table.add_row("Geliştirici", "46 ~~ @Tasdemir46")
