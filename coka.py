...
TOKEN_FILE = 'SLOV META TOOL Ä°NSTA HÄ°T.txt'
...
\033[1;34mââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
                      {banner}
\033[2;37m Ä°nstagram Meta Tool | telegram-> @slovyedek3
\033[1;34mââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
''')

ID = input('\033[1;33m -  ID GÄ°R REÄ°S : ')
...
TOKEN = input('\033[1;33m - TOKEN GÄ°R REÄ°S: ')
os.system('clear')
try:
...
def update_stats():
    # @slovyedek3 ;
    sysdontwrite = f"""\r{C1}Hit DÃ¼ÅtÃ¼{P1} : {hits}{J21} |{Z} KÃ¶tÃ¼ IG{P} : {J21}{bad_insta}{P} | {Z}KÃ¶tÃ¼ Mail{B} : {J21}{bad_email}{Z} | {P}Ä°yi IG{Z} : {J21}{good_ig}"""
...
def InfoAcc(username, domain):
    ...
    total_hits += 1
    info_text = f"""
ðð¢ð­ ðð®Ìð¬ð­ð®Ì ðð¬ð¥ðð§
ââââââââââââââ
ðð¶ð : [ {total_hits}  ]
ð¡ð¶ð°ð¸ð»ð®ðºð² : [ {username} ]
ððºð®ð¶ð¹ : [ {username}@{domain} ]
ðð¼ð¹ð¹ð¼ðð²ð¿ð : [ {followers} ] 
ðð¼ð¹ð¹ð¼ðð¶ð»ð´ : [ {following} ]
ð£ð¼ððð : [ {posts} ]
ðð¶ð¼: [ {bio} ]
ð¥ð²ðð²ð : [ {rest(username)} ]
ââââââââââââââ
[ ð¬ð®ð½ððºð°ð : @slovyedek3 ]
"""
