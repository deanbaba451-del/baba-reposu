...
COOKIE_VALUE = 'mid=ZVfGvgABAAGoQqa7AY3mgoYBVzP; csrftoken=9y3N5kLqzialQA7z96AMiyAKLMBWpqVj'
...
_2020py_header_ = render('2020 ', colors=['blue', 'gray'], align='center')
print("\x1b[1;36m" * 67)
print(_2020py_header_)
print("\x1b[1;36m" * 67)
...
def pppp():
    ...
    be = good_ig          
    print(f"\r          {S}  {S}Hits : {M}{ge} --   {E}KÃ¶tÃ¼ {E}: {M}{bt}  -- {O}KÃ¶tÃ¼ reset {O} : {M}{be}  | {R} \r", end='')
...
def rest(user):
    try:
        ...
        _2020zort_ = response.get('email', 'h h h')
    except:
        _2020zort_ = 'h h h'
        try:
            if '@' in _2020zort_:
                rest_user = _2020zort_.split('@')[0]
                if not (rest_user[0].lower() == user[0].lower() and rest_user[-1].lower() == user[-1].lower()):
                    _2020zort_ = 'h h h'
        except:
            _2020zort_ = ' h h h'
    return _2020zort_
...
------
Hit:{total_hits}
ðKullanÄ±cÄ± adÄ±:@{username}
ðTakipÃ§i:{followers} 
ðTakip:{following}
ðTarih:2020
ðPost:{posts}
ðReset: {rest(username)}
------



"""
    with open('2020Hits.txt', 'a') as f:
        f.write(info_text + "\n")
    try:
    ...
    while True:
        data = {
            ...
            'variables': json.dumps({
                'id': int(random.randrange(21254029834, 43464475395)),
        ...
        try:
            ...
            if username:
                ...
                if followers < 15: #minimum takipÃ§idir kafanÄ±za gÃ¶re deÄiÅtirin.
                    continue
                infoinsta[username] = account
                emails = [username + _2020mail_]
                for email in emails:
                    check(email)
        except Exception:
            pass
