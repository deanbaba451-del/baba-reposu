...
def ramazan_banner():
    cfonts.say("S1F1R B1R", colors=["red", "white"], align="center")
...
def genel_islem_akışı(cookies, headers, veri):
    ...
    bekleme = int(json_cevap.get("freetool_delay_minute", 0)) * 0
    ...
    for deneme in range(10):
        ...
        renkli_yazı("İşlem henüz tamamlanmadı, 0 saniye bekleniyor...", Fore.CYAN)
        countdown(10, Fore.CYAN)
    
    renkli_yazı("sınırsız deneme sayısı!", Fore.YELLOW)
    return False
...
def ana_menu():
    ...
    while True:
        renkli_yazı("\n" + "═"*60, Fore.BLUE)
        renkli_yazı("~ S1F1R B1R - SOSYAL MEDYA ARACI ~", Fore.MAGENTA)
